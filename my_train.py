...
def k_fold(args):
  ...
  #k_dataset = load_data("./train_btl_eng_pos400_comb300.csv")
  k_dataset = load_data("./train_entity850.csv")
  #k_dataset = load_data("./train_220329.csv")
  ...
  for n_iter, (train_ind, test_ind) in enumerate(kfold.split(k_dataset,k_label)):
   ...
   # trainer.hyperparameter_search(direction="maximize", hp_space=my_hp_space_ray)
    ...
    # train model
    trainer.train()
    model.save_pretrained(f'./best_model/{args.case_name}/{args.model_name}')
...
def main(args):
    wandb.login()
    wandb.init(project="KLUE_MODEL", entity="boostcamp_nlp06", name=args.name)
    torch.cuda.empty_cache()  
    if args.k_fold:
        k_fold(args)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default="klue/roberta-large", help='default: roberta-large')
    parser.add_argument('--k_fold', type=bool, default=False, help='default: False')
    ...
    parser.add_argument('--name', type=str, default="new test", help='wandb에 표시할 이름! 모델명-kfold5-기타')
    parser.add_argument('--case_name', type=str, default="last", help='모델을 저장할 폴더의 이름! default: last')
    parser.add_argument('--lr_sch', type=str, default='linear', help='LR 스케쥴러 cosine, linear, cosine_with_restarts default:linear')
